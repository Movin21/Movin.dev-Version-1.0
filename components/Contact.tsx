...
const Contact = () => {
  const [state, handleSubmit] = useForm("xwpepjvd");
  const formRef = React.useRef(null);

  if (state.succeeded && formRef.current !== null) {
    (formRef.current as HTMLFormElement).reset(); // Reset the form
  }
          ...
          <form className="my-8" onSubmit={handleSubmit} ref={formRef}>
            ...
            <LabelInputContainer className="mb-4">
              ...
              <Input
                id="email"
                name="email"
                placeholder="movin_21@movin.dev"
                type="email"
              />
              <ValidationError
                prefix="Email"
